...
def crashes_by_month(crashes):
    ...
    months = [datetime.date(1900, monthint, 1).strftime('%B') for monthint in month_ints]
...
def crashes_by_day(crashes):
    ...
    sns.despine(left=True, bottom=True)
    ax.set_xticklabels(list(calendar.day_name)[-1:]+list(calendar.day_name)[:6], rotation=45)
...
def crashes_by_pedpedal_action(people):
    ...
    ax.set_yticklabels(people.pedpedal_action.value_counts().sort_values().index)
    ax.set_xlabel('Number of Crashes')
    ax.set_ylabel('Bicyclist Action')
    ax.set_title('Number of Crashes based on Bicyclist Action (before Crash)', size=15)
    sns.despine(left=True, bottom=True)
...
def airbag_deployment(people):
    ...
    people_ab = people[(people['airbag_deployed']!='DEPLOYMENT UNKNOWN')&(people['airbag_deployed']!='NOT APPLICABLE')]
    ...
    crash_no_inj = people_ab[(people_ab['injury_classification'] == 'NO INDICATION OF INJURY')|(people_ab['injury_classification'] == 'REPORTED, NOT EVIDENT')]
    crash_inj    = people_ab[(people_ab['injury_classification'] != 'NO INDICATION OF INJURY')&(people_ab['injury_classification'] != 'REPORTED, NOT EVIDENT')]
